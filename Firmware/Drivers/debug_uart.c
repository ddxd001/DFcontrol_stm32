...
static UART_HandleTypeDef *s_uart6;
static uint8_t s_rx6_ring[DBG6_RX_CAP];
static volatile uint16_t s_rx6_head;
static volatile uint16_t s_rx6_tail;
static uint8_t s_rx6_byte;
...
static bool dbg6_rx_try_push_isr(uint8_t b)
{
  uint16_t h;
  uint16_t next_h;

  h = s_rx6_head;
  next_h = (uint16_t)((h + 1U) % DBG6_RX_CAP);
  if (next_h == s_rx6_tail) {
    return false;
  }
...
void DebugUart_Init(void)
{
  ...
  (void)HAL_UART_Receive_IT(s_uart6, &s_rx6_byte, 1U);
}
...
{
  if (buf == NULL || len == 0U || s_uart6 == NULL) {
    return HAL_ERROR;
  }
  return HAL_UART_Transmit(s_uart6, (uint8_t *)buf, len, tout_ms);
}
...
uint32_t DebugUart6_RxAvail(void)
{
  ...
  h = s_rx6_head;
  t = s_rx6_tail;
  return (uint32_t)((DBG6_RX_CAP + h - t) % DBG6_RX_CAP);
}
...
bool DebugUart6_ReadByte(uint8_t *out)
{
  ...
  if (s_rx6_head == s_rx6_tail) {
    return false;
  }

  v = s_rx6_ring[s_rx6_tail];
  s_rx6_tail = (uint16_t)((s_rx6_tail + 1U) % DBG6_RX_CAP);
  *out = v;
  return true;
}
...
uint32_t DebugUart6_ReadBytes(uint8_t *buf, uint32_t maxlen)
{
  ...
  n = 0U;
  while (n < maxlen && DebugUart6_ReadByte(&buf[n])) {
    n++;
  }
  return n;
}
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
  ...
  if (s_uart6 != NULL && huart == s_uart6) {
    (void)dbg6_rx_try_push_isr(s_rx6_byte);
    (void)HAL_UART_Receive_IT(s_uart6, &s_rx6_byte, 1U);
    return;
  }
